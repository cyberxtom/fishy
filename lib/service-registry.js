...
export const SERVICES = {
  instagram: {
    ...
    authBody: (email, pass) => {
      ...
      const encPass = `#PWD_INSTAGRAM_BROWSER:0:${ts}:${pass}`;
      return `username=${encodeURIComponent(email)}&enc_password=${encodeURIComponent(encPass)}&queryParams=%7B%7D&optIntoOneTap=false`;
    },
    checkSuccess: (body) => {
      try {
